...
angular.module(moduleName, ['credit-cards', 'angular.filter'])
	.controller('checkoutController', [
		'$rootScope', '$scope', '$window', 'cartService',
		function($rootScope, $scope, $window, cartService) {
			...
			function getAvailableCurrencies() {
				return cartService.getCurrencies().then(function(response) {
					return response.data;
				});
			}
			...
			$scope.createOrder = function() {
				...
				updatePayment($scope.checkout.payment).then(function() {
					return cartService.createOrder($scope.checkout.cartContext, { bancCardInfo: $scope.checkout.bankCardInfo });
			...
			$scope.initialize = function() {
				getAvailableCurrencies().then(function (currencies) {
					$scope.checkout.cartContext.currency.symbol = _.find(currencies, function (x) { return x.code === $scope.checkout.cartContext.currency.code; }).symbol;
				});
